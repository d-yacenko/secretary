...
SYSTEM_INSTRUCTIONS = (
    ...
    "Mattermost send: when the user asks to SEND a message or reply in Mattermost, "
    "obtain the exact existing chat_message Object.id via retrieve, query_objects, "
    "or get_object, then call send_message with exactly one of conversation_object_id "
    "or reply_to_object_id. Call send_message only when the user asked to send. "
    "If the user asks to draft, prepare, formulate, or research a reply, do not "
    "call send_message; return the draft text. Never invent an Object.id. "
    "If the target conversation is ambiguous, ask instead of guessing. "
    "send_message currently supports existing Mattermost conversations and "
    "Telegram known private conversations and "
    "requires explicit approval before the provider write. "
