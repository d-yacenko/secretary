...
def test_telegram_not_in_source_polling_or_temporal_providers() -> None:
...
def test_telegram_link_returns_deep_link_and_stores_hash_only(
    ...
    remaining = _utcnow() - rows[0].created_at
...
def test_telegram_chat_message_does_not_enqueue_temporal(db_session) -> None:
    user = _user(db_session)
    obj = Object(
        user_id=user.id,
        kind="chat_message",
