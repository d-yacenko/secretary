...
- Worker lanes (Voice A Final Inbox/Source Corrective, not CODE ACCEPTED / not deployed): each `RECURRING_SOURCE_JOB_TYPES` entry has an isolated `worker-source-{job_type}` thread; the general lane excludes those types plus `run_scheduled_activity`. A hung Yandex IMAP handler must not starve Gmail/Teams `claim_next`. Yandex `IMAP4_SSL` uses a 30s socket timeout; connect/command `OSError` / `IMAP4.error` fail closed.
...
- **Status:** **CODE ACCEPTED / MANUALLY VERIFIED** at application SHA `7ae954aa186ee211e6640753e384de6e1c75a93d` on `review/pre-voice-ui-corrective-a`. Voice Assistant A is unblocked.

## Voice Assistant A — turn-based hands-busy voice loop

- **Motivation:** voice is an interface over the existing Secretary Assistant, not a second agent. Hands-busy turns use the same conversation, tools, Policy Gateway, frozen Pending Action Plan, approval endpoints, and Execution Gateway as typed Assistant.
- **TTS:** authenticated `POST /assistant/speech` synthesizes audio bytes. Input is `{"text": "..."}`. Output is audio bytes with explicit `Content-Type`. Synthesized audio and raw voice audio are not persisted as Secretary Objects/resources.
- **Provider:** `SpeechProvider` + OpenAI implementation + fake provider. Same effective per-user OpenAI credential resolution as transcription. Model `OPENAI_TTS_MODEL` (default `gpt-4o-mini-tts`). Voice is configurable via `OPENAI_TTS_VOICE`; product behavior does not depend on a built-in voice name.
- **Bounds:** Secretary-owned max input per synthesis request is 3000 Unicode characters. Blank or over-limit direct API input is typed 422 (`speech_text_empty` / `speech_text_too_long`). Validation/prepare runs before resolving the user OpenAI credential or constructing the budget-guarded speech provider, so invalid input does not require provider configuration and makes zero provider/OpenAI calls. Valid text with missing credential still returns safe 502. Long Assistant answers are chunked client-side at paragraph/sentence boundaries, then synthesized/played in order. TTS preprocessing strips presentation-only Markdown; it does not summarize or rewrite the Assistant answer. On-screen Assistant text remains authoritative.
- **Cost Guard:** every paid speech call passes `OpenAIDailyBudgetGuard` before the provider request. AI audit workload `speech`. Token usage is recorded only when the provider/SDK reports actual usage. Character→token estimates are not invented. Exhausted budget returns the same typed interactive 429 as STT/Assistant, with no provider call.
- **Playback:** Flutter `audioplayers` (Android + Linux, `stop()` immediately, local file source). Chosen because the existing stack cannot play returned audio, it preserves Android minSdk 23 (plugin minSdk 19, guaranteed support ≥ 23), and it is not a full media framework. Temp playback files are deleted after complete/stop/error/dispose. No permanent voice cache. Current Flutter plugin default `minSdkVersion` is 24; Voice A keeps API 23 via `android.defaultConfig.minSdk = 23` because `flutter build` rewrites `defaultConfig.minSdk` to `flutter.minSdkVersion`. Linux playback compiles against GStreamer (`audioplayers_linux`). R1 Linux `flutter build linux --debug` succeeded in a temporary openSUSE Tumbleweed container with `gstreamer-devel` / `gstreamer-plugins-base-devel` (not production VDS). This development host has CMake/clang and GStreamer **runtime** 1.26.7; `gstreamer-devel` is still not installed on the host. Microphone runtime tools (`parecord` / `pactl` / `ffmpeg`) remain separate from playback build deps. Existing `flutter_secure_storage_linux` needs `libsecret` devel.
- **Session:** turn-based, button-started. Visible states: `idle`, `recording`, `transcribing`, `thinking`, `speaking`, `error`. Microphone starts every utterance. Auto-speech is a frozen per-turn output policy, not “any microphone input”. Typed turns never auto-speak.
- **R4-R2 invocation source / local auto-speech (refined R4-R4-R2 + Lock-screen Launcher A):** `VoiceInvocationSource` is `typed` | `screenMic` | `hardwareButton` | `systemAssistant` | `lockScreenLauncher`, frozen at recording start; a later stop gesture cannot promote or demote it. `lockScreenLauncher` is `isVoiceInput=true` and `isHandsFree=true` (not an overload of `systemAssistant`). Device-local SharedPreferences `voice_output_policy.$userId` is not UserSettings. Canonical stored values: `hands_free_enabled` (default) / `never`. Product invariant: `screenMic` and typed never auto-speak; no setting may re-enable screen-mic TTS. `handsFreeEnabled` speaks hardware / system-assistant / lock-screen launcher. `never` speaks none. Legacy `all_voice_input` and `hands_free_only` decode and rewrite to `hands_free_enabled`. Account UI is a two-state local switch, not a three-way radio. If auto-speech is off for the turn, Pending Action Plan is visual-only, affirmative «Да» stays unarmed, exact «Нет» may still reject. Screen-mic Inbox review may show a receipt but does not auto-complete the marker. `audioplayers` keeps default Android media routing; no custom Bluetooth stack.
- **R4-R2 transcription observability:** production phone failures were valid short PCM WAVs (about 80–400 ms) rejected by OpenAI 400 / empty text, previously collapsed to 502 `"Transcription provider unavailable"`. Client rejects WAV shorter than 500 ms locally. Backend (not deployed with R4-R2) returns typed `transcription_provider_not_configured` / `transcription_provider_failed` / `transcription_audio_invalid`. Do not silently fall back to a different OpenAI secret. Do not force a new recorder encoder without evidence the container/MIME is wrong. Screen mic and hardware share one transcription pipeline.
- **Interruption:** microphone during normal TTS stops playback, discards queued chunks, and starts a new recording. Explicit Stop stops playback and leaves the conversation intact. No acoustic barge-in.
- **Pending-plan narration:** spoken approval preview is built from frozen plan `arguments` (not Assistant prose). `send_email` speaks recipients, subject, exact body. `send_message` speaks provider, frozen route identity, reply/new when available, exact body. Ends with «Отправить?». Mixed/unsupported plans stay visual-only; voice approval is disabled.
- **Voice approval:** fail-closed deterministic whole-command parser. No LLM/fuzzy confirmation. Affirm: `да`, `отправляй`, `подтверждаю`, `да отправляй`. Reject: `нет`, `не отправляй`, `отмена`. Normalization: trim, lowercase, collapse whitespace, strip terminal punctuation. Ambiguous (`да, но …`, `наверное да`) does not approve or send the transcript to Assistant.
...
- **R3 hardware trigger (Android, foreground only):** the physical button is a local-device binding, not a backend user setting. Persist in client `shared_preferences` as `hardware_voice_binding.$userId` (`enabled`, `keyCode`, `scanCode`, `gesture` `single`|`double`, `displayLabel`). Do not persist `deviceId`. Logout disables the native MethodChannel binding immediately and keeps the namespaced pref. Learn captures the next eligible foreground `KeyEvent` (timeout 9000 ms) without OEM/Bixby hard-coding; unknown vendor keys display as `Дополнительная кнопка (код N)` unless Android supplies a trustworthy label. Rejected Learn keys: HOME 3, BACK 4, CALL 5, ENDCALL 6, VOLUME_DOWN 25, POWER 26, MENU 82, VOLUME_MUTE 164, APP_SWITCH 187, SLEEP 223, WAKEUP 224, SOFT_SLEEP 276, SYSTEM_NAVIGATION_* 280–283. Generic extra keys default to single press; Volume Up is forced to double press. R4 sets the double-press window to **500 ms** (was 350 ms) from user timing evidence; no extra user setting. Volume Up single delayed press uses public `AudioManager.adjustSuggestedStreamVolume` with `FLAG_SHOW_UI | FLAG_REMOVE_SOUND_AND_VIBRATE`; double press emits one `onVoiceTrigger` and does not also raise volume. Long-press repeats raise volume and do not start Voice. Screen mic and hardware share `AssistantController.handleVoiceTrigger()`. If AppShell is not the current route or `Navigator.canPop()`, ignore hardware Voice rather than popping Account/detail/capture/OAuth. Learn/Test consume the gesture and do not start Voice. Linux has no hardware-button selector. Channel `secretary/hardware_voice` protocol `secretary.hardware_voice.v1` (`setListening` added in R4 so native start/stop earcons follow recorder state). Flutter `getStatus` handshake; native configure ack is authoritative for ACTIVE vs saved-but-inactive. Learn/Test statuses include `bridgeError` distinct from timeout. Do not present OEM intercept if the native plugin is missing. Debug logcat tags `SecretaryHwVoice` and `SecretaryVoiceTiming`. minSdk remains 23.
- **R4 local feedback / system assistant / lock screen:** hardware / keyguard acknowledgement is a short native haptic + tiny tone only (“invocation recognized”). R4-R3: the audible ready cue, when used, completes **before** the microphone starts (“speak after the tone”). Screen mic uses UI/haptic only. Stop/process plays only after recorder stop. Native hardware no longer plays STREAM_MUSIC stop while capture is active. System-assistant `VoiceInteractionSession` must not play a ready cue before recorder readiness. Volume Up double remains foreground-only. **Lock-screen Launcher A product path:** Google/Gemini remains the Android default assistant. Secretary must not require `ROLE_ASSISTANT`, default `VoiceInteractionService`, Accessibility, global volume-key interception, a wake word, or always-listening microphone. Account no longer presents «Выбрать Секретарь помощником…» and does not call `requestAssistantRole` from ordinary Profile UI or when opening driving mode. Dormant VoiceInteractionService / role-request native code may remain but is not the setup path. The user enables per-user `lock_screen_voice_enabled.$userId` (default off) and taps «Открыть режим вождения», which starts `VoiceSessionActivity` in typed `launcher` mode (no `EXTRA_VOICE_TRIGGER`, idle until the large button). Existing `assistInvoke` + `secretary.voice_trigger` auto-record remains for dormant system-assistant invokes. Overlay uses `showWhenLocked` / `turnScreenOn` without `FLAG_KEEP_SCREEN_ON`; `onStart` re-applies flags. The device may sleep. If Android kills the activity/process, automatic resurrection is not required in Launcher A. No SCREEN_ON relaunch, no ongoing foreground service, no lock-screen notification fallback in A. Unauthenticated overlay shows only a safe sign-in-required message, never `AuthSetupScreen`. Locked overlay is a minimal voice surface, not Inbox/history. External writes cannot approve/execute while keyguard is locked; exact reject remains allowed. Linux plugin libraries must use `RUNPATH=$ORIGIN` so `libduckdb.so` resolves without `LD_LIBRARY_PATH`. Do not use Accessibility, root, adb remapping, private Samsung APIs, fake MediaSession volume hacks, or a wake word. **Launcher A R1 UI polish:** only `VoiceSessionApp` uses `ThemeMode.system` (light + dark `ThemeData`); the ordinary Secretary app stays light-only. Driving Account section is Android-only and sits immediately after Profile / Owner. Device-local UI text scale min `0.50` / default `1.00` / max `1.30` with 5% slider steps; stored values clamp to those bounds. Inbox review marker accent is `Color(0xFFFF4D2E)` (~2 px divider, semibold label). Review-rail click is available on Android/iOS/Linux/Windows/macOS; Swipe-to-Remove remains `inboxUsesTouchReviewRail` (Android/iOS only). Desktop drag remains a secondary affordance. Marker persistence semantics are unchanged. **Launcher A R1 locked communication:** voice is not biometric identity. Security boundary is an explicit Driving Mode launch from the authenticated unlocked app. Authorization is an in-memory session nonce (`DrivingVoiceSessionRegistry` + intent extra), not SharedPreferences/server, and does not survive process death, close, or auth loss. `assistInvoke` and already-locked open do not arm it. Canonical `mayVoiceApproveLockedPendingPlan` is the only locked voice-approval bypass: same active session, `lockScreenLauncher` source, one pending plan, `planIsVoiceApprovable` (`send_email`/`send_message` only). Full frozen narration must finish before exact «Да» executes through the normal approval path while still locked. Visual tap-to-approve and all other locked writes stay fail-closed. **Final Client Shortcut Completion:** Android-only `Icons.directions_car` next to the existing Account action in the main shell (narrow AppBar left of Account; wide NavigationRail trailing above Account); tooltip «Режим вождения». Linux/desktop has no car shortcut. If `lock_screen_voice_enabled.<userId>` is true, tap opens the existing Driving / Lock-screen Launcher once. If false, compact confirmation «Разрешить голосовой режим на экране блокировки?»; Cancel changes nothing; «Включить и открыть» persists through existing `SystemAssistantController.setLockScreenVoiceEnabled` then the same launcher once. The Account Driving Mode section remains. No second launcher, no `ROLE_ASSISTANT`, Google remains the default assistant.
- **R4-R3 recorder / audio session:** OpenAI rejecting <500 ms is a downstream consequence, not the recorder defect. Production Voice must not play `audioplayers` media between successful recorder start and successful recorder stop. Hands-free ready waits for `onPlayerComplete`, then releases the player, then starts the mic. Stop recorder first, then stop cue. Debug capture metadata (source, monotonic timestamps, file bytes, WAV duration, wall-clock interval) never includes audio/transcript/token. After `stop()`, wait until the file is stable only if a 50 ms sample shows growth, timeout 500 ms. Do not add a global trigger debounce unless duplicate-trigger evidence exists. TTS/`audioplayers` from a prior turn must be released before the next microphone start. 500 ms client WAV floor remains defensive validation with duration in the debug/user-test error copy.
- **R4-R3 transcription taxonomy:** locally proven malformed/too-short Secretary WAV → `transcription_audio_invalid` (422). Empty successful provider text → `transcription_unrecognized` (422). Generic provider HTTP 400 is `transcription_provider_failed` (502), not “recording too short”. No provider bodies or secrets to the client.
- **Inbox review marker (R2, refined R4-R4 / R4-R4-R1):** the persisted Inbox «Просмотрено досюда» frontier is a provider-neutral Secretary annotation (`inbox_review_markers`), not Gmail/Yandex/Mattermost/Telegram/Teams read/unread. Assistant tools: `list_inbox_since_review_marker` (READ), `set_inbox_review_marker` and `clear_inbox_review_marker` (ANNOTATE). “New since marker” is Inbox-eligible objects strictly newer than `(anchor_feed_at, anchor_object_id)`; the inclusive anchor itself is not new. No marker → explicit not-set. Inbox UI / `RecentSourceService.list_page` remain newest-first (`feed_at DESC, object_id DESC`). ANNOTATE executes immediately in interactive Assistant (no Pending Action Plan, no external write); `after_object_id` must have been exposed this turn. MCP ANNOTATE remains fail-closed. No Flutter phrase parser. R2-R1: candidate/reference IDs are only new `items`; `anchor_object_id` stays in seen IDs but is not an answer reference. R4-R4: listing paginates a frozen snapshot (`old_marker < item <= snapshot_top`) with an opaque continuation cursor and exact `total_count`. Typed tool `purpose` is `inspect` (newest-first count/peek; never a completion receipt) or `review` (complete listening of that frozen snapshot). R4-R4-R1: first review page discovers `snapshot_top` as the newest eligible object, then returns chronological pages (`feed_at ASC, object_id ASC`) from the old end; continuation is strictly newer than last emitted, still `<= snapshot_top`. Cursor v=2 binds `dir` (`asc`/`desc`); incompatible/v=1 cursors fail closed. Review `remaining_count` is items strictly newer than last emitted. Inspect, count-only, partial pagination, tool-budget exhaustion, mixed/malformed cursors, and LLM prose do not move the marker. A verified `inbox_review_receipt` may advance the marker to **frozen snapshot_top** (newest frozen item) only after that turn’s frozen VoiceOutputPolicy allows auto-speech and all TTS chunks finish without interrupt/error, via `POST /inbox/review-marker/complete` CAS. Arrivals above snapshot top stay new after completion. Complete-enumeration utterances are `purpose=review` even when narration is only sender/source + subject/title + optional short excerpt (full body not required). Backend grammar (R2-R1): review verb (`перечисли`/`назови`/`прочти`/`прочитай`) + optional harmless filler (`мне`, `пожалуйста`, …) + «все новые сообщения», plus «что нового / перечисли всё». Count/peek remains `purpose=inspect`: «сколько» + optional «у меня»/«сейчас» + «новых сообщений», plus peek forms «сколько новых?», «покажи последние два», «есть что-нибудь новое?». Review wins if one utterance asks both. No Flutter phrase parser and no generic NLP. **Final Production Corrective R2:** `AssistantService`/`PerTurnToolBudget` force that classifier onto `list_inbox_since_review_marker` so the LLM cannot downgrade explicit enumeration to `inspect` or upgrade count/peek to `review`; the model still chooses whether to call the tool. Verified receipt is computed from domain tool output of the frozen snapshot, not from char-truncated model-visible pages. Client completion stays fail-closed and records stage diagnostics without message content. Interrupt, screenMic enumeration, and count-only do not auto-complete. Physical marker proof requires the combined new backend; a new Voice A client against old production (no R4-R4 receipt contract) is not a client-bug proof. The lock icon in the visual reference has no product meaning; do not add it. Visual-identity orb redesign remains backlog-only.
- **Inbox Conversation Compaction A:** implemented as presentation overlay on `review/unified-conversation-compaction-a` (not CODE ACCEPTED, not deployed). Graph Refinement remains the next heavy identity phase. Out of scope for Voice Assistant A / R4-R4 / R4-R4-R1 code reopen.
- **Status:** **CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE** at application SHA `7610cc1c6ce24764fae73317a6925cdce7bd27b2` on `review/voice-assistant-a` (previous production `b0c75eaa5e879ee108afe90f152a29914d28a2f2`; R2-R1 parent `44161b01b8b065204de0f5025fc55b96910f6058`; R2 `244cdd28b81e7d8b33b6f78938fa062c03a82b3a`; R1 PASS `89c7e0bf9fc74dbfcc8a02f9a6795b4f3deae82e`). **R3 not CODE ACCEPTED.** **R4 not CODE ACCEPTED.** **R4-R1 not CODE ACCEPTED**; R4-R1 application SHA `e097069dad8bcb0066e531a63f5f668e786e62ab`. **R4-R2 not CODE ACCEPTED**; R4-R2 application SHA `f8c9a256af9d07b77b088e94fc1110e69479343f`. **R4-R3 not CODE ACCEPTED**; R4-R3 application SHA `d121cf90e351612275663c6845a76e9dd80ed587`. **R4-R4 included in deployed combined tree**; R4-R4 application SHA `dc3568d2cb2e4808f4a736933b0ce615553a11eb`. **R4-R4-R1 included in deployed combined tree**; R4-R4-R1 application SHA `0ab2cb6f1ae2a373f8b719314777824434c9adda` (parent `c4bcd66020d747668eb99ea03ea7613814e4b8d8`; rejected R4 `a4bb618c69ff170f223bef13dd5303d8b5961b67`; R3-R1 base `f20c0e4bcc55bbb9e6a4b79cf052d17f8daf637b`; R3 `fe496d23358fc3bd0b0cd407fc72258ceeec0022` rejected by user-manual evidence; R3 docs tip `03cb692d0a032f2e069d3f5718dcc95bfeb41f03`). **R4-R4-R2 included in deployed combined tree**; R4-R4-R2 application SHA `276fe709527a7d300660cbe444ae40ee520e7e05` (parent `09f58b9ebf261df728352faf239ff2759cf12031`). **Lock-screen Launcher A / R1 functional completion included in deployed combined tree**; Lock-screen Launcher A R1 application SHA `319a99e5c2d55750fd9949f8a31fd137c45b703b` (accepted UI polish parent `b99bbad50c7d0d8a8edbef8d05781a2c7fb9de23`; previous docs tip `489eb9c1300fac11e6766a9cc155d129cb9cfd2a`). **Final Inbox/Source Corrective included in deployed combined tree**; application SHA `f50b56c6d874148963479830ad25af5b65522c20` (parent `319a99e5c2d55750fd9949f8a31fd137c45b703b`). **Final Client Shortcut Completion CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE**; application SHA `b0c75eaa5e879ee108afe90f152a29914d28a2f2` (parent docs tip `afaab5859b57c5a34c9dcb5eb14f92e790dcbae0`; accepted backend/source base `f50b56c6d874148963479830ad25af5b65522c20`). R1 PASS. R2 marker architecture/semantics PASS (auto-move refined in R4-R4; review order chronological in R4-R4-R1). R2-R1 reference hygiene PASS. **Final Production Corrective R2 not CODE ACCEPTED** at `38d1e1a281f4f129cfc3d56924f193d85dd96c90` (lastUpdatedDateTime skip blocker). **R2-R1 direction Architect-accepted**, SHA `30fb7553ec343764dfcd8eb69b8316f9431b0b92` **not CODE ACCEPTED** (permanent notification job dedupe). **R2-R2 CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE**, not PRODUCTION ACCEPTED, at application SHA `7610cc1c6ce24764fae73317a6925cdce7bd27b2` (parent `acdff76dc86cfe5bd4ed82e1a5c619f480030afe`; previous production `b0c75eaa5e879ee108afe90f152a29914d28a2f2`). Production Alembic **0040 → 0041**. Do not skip `list_chat_messages` from `Chat.lastUpdatedDateTime` (rename/membership only). Safe no-change skip is Graph v1.0 `GET /me/chats?$expand=lastMessagePreview` when parsed `lastMessagePreview.createdDateTime <=` cached `last_created_at`; otherwise fail open. Webhook remains primary; 900s reconciliation remains recovery. `reauthorizationRequired` marks `reauthorization_required` and queues sync; `ensure()` PATCHes expiration even outside the 15-minute window; PATCH 404 recreates. Targeted Graph `created`/`updated` notifications each enqueue `process_teams_notification` (in-batch identical collapse only; no historical DONE/FAILED `enqueue_once`). Unexpected `changeType` is ignored. `upsert_message` stays idempotent by `external_id`. Graph v1.0 `/users/{microsoft-user-id}/chats/getAllMessages` create on the connected tenant returned HTTP 201 (non-rich); probe subscription deleted. Production Alembic **0040 / 0040**; corrective tree still **0041** `teams_subscriptions`. Debug APK sha256 `e6933829d12ab3ea7f4793c92c00e5a6394c339aca24bbe618b444e1e18305c9`, aapt `sdkVersion:'23'`. Not PRODUCTION ACCEPTED. Do not start Voice B. Assistant `ai_traces_user_id_fkey` failures in `test_assistant.py` / `test_assistant_action_plans.py` remain PRE-EXISTING BASELINE / NONBLOCKING.

## Unified Conversation & Inbox Compaction A

- **Motivation:** flatten communication-heavy Inbox rows into short Conversation Stacks without changing source acquisition or inventing conversation Objects.
- **Projection:** provider-neutral Inbox/Voice projection only. Gmail `account+thread_id`; Yandex RFC Message-ID / In-Reply-To / References with conservative subject+participants fallback; Telegram/Teams existing `chat_id`; Mattermost `root_id` or D/G channel identity; public unthreaded Mattermost is coarse and uses temporal/participant segmentation.
- **Burst:** adjacent messages in the same conversation identity, gap ≤ 15 minutes. Role reversal does not split. Singleton remains a normal row. False merge is worse than split.
- **Presentation:** overlay `conversation_groups` on `/inbox` and `/inbox/feed`. Flat `recent_source_objects` / feed items and underlying-object cursors remain. Stacks are not Objects and create no graph edges.
- **Marker:** defined only by underlying Object/`feed_at`. A candidate stack that crosses the marker is split; collapsed UI never hides the red boundary.
- **Summary:** `Representation(kind=conversation_stack_summary)` on the newest/anchor member, valid only for exact stack fingerprint. Multiple fingerprint variants may coexist on one anchor (retention cap 8). Async job `summarize_conversation_stack`; pending/running deduped by fingerprint; recent FAILED (15 min cooldown) does not enqueue; older FAILED may retry once.
- **Voice:** same review-marker/snapshot/CAS machine. `total_count` remains raw objects. `conversation_count` is exact whole-frozen-snapshot grouping count when `conversation_count_exact` (in-process, ≤200 objects); `page_conversation_count` is this page only. Preferred «N в M» only when exact. Receipt credits only model-visible compact/item object_ids; serializer compact is trimmed to the same visible page.
- **Deferred:** Telegram transport/groups; source acquisition changes; cross-provider identity merge; Graph Refinement A; Voice B.
- **Status:** **Architect R1 corrective implemented / awaiting Architect review**, not CODE ACCEPTED, not PRODUCTION ACCEPTED, not deployed. Branch `review/unified-conversation-compaction-a`. Canonical base `54b820d01d97c1ae44fba3d2d5831e1605e9c5d7`. Production remains `7610cc1c6ce24764fae73317a6925cdce7bd27b2`, Alembic **0041**. Migration **NONE**.
