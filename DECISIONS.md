...
- **Inbox review marker (R2, refined R4-R4):** the persisted Inbox «Просмотрено досюда» frontier is a provider-neutral Secretary annotation (`inbox_review_markers`), not Gmail/Yandex/Mattermost/Telegram/Teams read/unread. Assistant tools: `list_inbox_since_review_marker` (READ), `set_inbox_review_marker` and `clear_inbox_review_marker` (ANNOTATE). “New since marker” is Inbox-eligible objects strictly newer than `(anchor_feed_at, anchor_object_id)` in canonical feed order (`feed_at DESC, object_id DESC`); the inclusive anchor itself is not new. No marker → explicit not-set; do not classify the whole historical Inbox as new. ANNOTATE executes immediately in interactive Assistant (no Pending Action Plan, no external write); `after_object_id` must have been exposed this turn. MCP ANNOTATE remains fail-closed. No Flutter phrase parser. R2-R1: candidate/reference IDs are only new `items`; `anchor_object_id` stays in seen IDs (legal annotation target) but is not an answer reference. R4-R4: listing paginates a frozen snapshot (`old_marker < item <= snapshot_top`) with an opaque continuation cursor and exact `total_count`. Typed tool `purpose` is `inspect` (count/peek; never a completion receipt) or `review` (complete listening of that frozen snapshot). Inspect, count-only, partial pagination, tool-budget exhaustion, mixed/malformed cursors, and LLM prose do not move the marker. A verified `inbox_review_receipt` may advance the marker to **frozen snapshot_top** only after that turn’s frozen VoiceOutputPolicy allows auto-speech and all TTS chunks finish without interrupt/error, via `POST /inbox/review-marker/complete` CAS (advance if current == expected anchor; already_current no-op; incompatible concurrent change is conflict / no overwrite). Fetching, Assistant backend response, textual rendering, start of TTS, interrupted speech, TTS/playback failure, screen-mic under default `handsFreeOnly`, and `never` do not auto-advance. Explicit set/clear remain independent. Arrivals above snapshot top stay new after completion.
- **Inbox Conversation Compaction A (backlog, not started):** presentation-level grouping only. Keep every underlying `chat_message` object unchanged. Group by exact provider + exact conversation/chat identity. Short consecutive bursts may render as one compact Inbox unit showing participant/chat, message count, time range, and a short summary; expandable/drill-down to original messages; exact original message IDs remain available for reply/provenance. Do not group cross-provider identities until Graph Refinement supports it safely. Out of scope for Voice Assistant A / R4-R4.
- **Status:** **CODE ACCEPTED / DEPLOYED / AWAITING USER MANUAL ACCEPTANCE** at application SHA `5ef2a2db117dc5a1c0ac54bad3d0e897ebcf6357` on `review/voice-assistant-a` (R2-R1; parent `44161b01b8b065204de0f5025fc55b96910f6058`; R2 `244cdd28b81e7d8b33b6f78938fa062c03a82b3a`; R1 PASS `89c7e0bf9fc74dbfcc8a02f9a6795b4f3deae82e`). **R3 not CODE ACCEPTED.** **R4 not CODE ACCEPTED.** **R4-R1 not CODE ACCEPTED**; R4-R1 application SHA `e097069dad8bcb0066e531a63f5f668e786e62ab`. **R4-R2 not CODE ACCEPTED**; R4-R2 application SHA `f8c9a256af9d07b77b088e94fc1110e69479343f`. **R4-R3 not CODE ACCEPTED**; R4-R3 application SHA `d121cf90e351612275663c6845a76e9dd80ed587`. **R4-R4 not CODE ACCEPTED**; R4-R4 application SHA `dc3568d2cb2e4808f4a736933b0ce615553a11eb` (parent `cfab062f22f6fca9acb1d0f33b580c901f42ed3c`; rejected R4 `a4bb618c69ff170f223bef13dd5303d8b5961b67`; R3-R1 base `f20c0e4bcc55bbb9e6a4b79cf052d17f8daf637b`; R3 `fe496d23358fc3bd0b0cd407fc72258ceeec0022` rejected by user-manual evidence; R3 docs tip `03cb692d0a032f2e069d3f5718dcc95bfeb41f03`). R1 PASS. R2 marker architecture/semantics PASS (auto-move refined in R4-R4). R2-R1 reference hygiene PASS. Production Alembic **0040 / 0040**. Not PRODUCTION ACCEPTED. Do not start Voice B. Assistant `ai_traces_user_id_fkey` failures in `test_assistant.py` / `test_assistant_action_plans.py` remain PRE-EXISTING BASELINE / NONBLOCKING.
