...
- **R4-R2 invocation source / local auto-speech:** `VoiceInvocationSource` is `typed` | `screenMic` | `hardwareButton` | `systemAssistant`, frozen at recording start; a later stop gesture cannot promote or demote it. Device-local SharedPreferences `voice_output_policy.$userId` (`handsFreeOnly` default / `allVoiceInput` / `never`) is not UserSettings. Default: screen mic is voice input only; hardware button and system/lock-screen assistant are hands-free. If auto-speech is off for the turn, Pending Action Plan is visual-only, affirmative «Да» stays unarmed, exact «Нет» may still reject. `audioplayers` keeps default Android media routing; no custom Bluetooth stack.
