...
- **Playback:** Flutter `audioplayers` (Android + Linux, `stop()` immediately, local file source). Chosen because the existing stack cannot play returned audio, it preserves Android minSdk 23 (plugin minSdk 19, guaranteed support ≥ 23), and it is not a full media framework. Temp playback files are deleted after complete/stop/error/dispose. No permanent voice cache. Current Flutter plugin default `minSdkVersion` is 24; Voice A keeps API 23 via `android.defaultConfig.minSdk = 23` because `flutter build` rewrites `defaultConfig.minSdk` to `flutter.minSdkVersion`. Linux playback compiles against GStreamer (`audioplayers_linux`); this host had GStreamer runtime but not devel headers, so Linux CMake verification did not complete here.
