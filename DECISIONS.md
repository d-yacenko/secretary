...
- **R4-R2 invocation source / local auto-speech (refined R4-R4-R2):** `VoiceInvocationSource` is `typed` | `screenMic` | `hardwareButton` | `systemAssistant`, frozen at recording start; a later stop gesture cannot promote or demote it. Device-local SharedPreferences `voice_output_policy.$userId` is not UserSettings. Canonical stored values: `hands_free_enabled` (default) / `never`. Product invariant: `screenMic` and typed never auto-speak; no setting may re-enable screen-mic TTS. `handsFreeEnabled` speaks only hardware / system / lock-screen assistant. `never` speaks none. Legacy `all_voice_input` and `hands_free_only` decode and rewrite to `hands_free_enabled`. Account UI is a two-state local switch, not a three-way radio. If auto-speech is off for the turn, Pending Action Plan is visual-only, affirmative «Да» stays unarmed, exact «Нет» may still reject. Screen-mic Inbox review may show a receipt but does not auto-complete the marker. `audioplayers` keeps default Android media routing; no custom Bluetooth stack.
...
- **R4 local feedback / system assistant / lock screen:** hardware / keyguard acknowledgement is a short native haptic + tiny tone only (“invocation recognized”). R4-R3: the audible ready cue, when used, completes **before** the microphone starts (“speak after the tone”). Screen mic uses UI/haptic only. Stop/process plays only after recorder stop. Native hardware no longer plays STREAM_MUSIC stop while capture is active. System-assistant `VoiceInteractionSession` must not play a ready cue before recorder readiness. Volume Up double remains foreground-only. Background / lock-screen hands-free uses public `VoiceInteractionService` + user-selected Digital Assistant (`ROLE_ASSISTANT` where `RoleManager` exists). The public keyguard entry is `VoiceInteractionService.onLaunchVoiceAssistFromKeyguard()`, not `VoiceInteractionSession.onShow()`. Repeated lock-screen invokes use `secretary.voice_trigger` + `VoiceSessionActivity.onNewIntent`. Overlay bootstrap waits for auth, the per-user lock-screen Voice pref, and keyguard state. Assistant-role UI refreshes after the system picker returns (`onActivityResult` / resume), including API 23–28 `VOICE_INPUT_SETTINGS`. Lock-screen Voice is a separate per-user opt-in default off. Locked overlay is a minimal voice surface, not Inbox/history. External writes cannot approve/execute while keyguard is locked; exact reject remains allowed. Linux plugin libraries must use `RUNPATH=$ORIGIN` so `libduckdb.so` resolves without `LD_LIBRARY_PATH`. Do not use Accessibility, root, adb remapping, private Samsung APIs, fake MediaSession volume hacks, or a wake word.
