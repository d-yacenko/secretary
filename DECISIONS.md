...
- **Summary:** `Representation(kind=conversation_stack_summary)` on the newest/anchor member, valid only for exact stack fingerprint. Async job `summarize_conversation_stack` on Inbox GET / review listing; Inbox GET does not block on LLM. Pending/running jobs deduped by fingerprint; FAILED does not permanently suppress retry.
- **Voice:** same review-marker/snapshot/CAS machine. Compact narration of stacks; `total_count` remains raw objects; `conversation_count` is separate. Detail resolves to underlying object ids.
- **Deferred:** Telegram transport/groups; source acquisition changes; cross-provider identity merge; Graph Refinement A; Voice B.
- **Status:** **implemented / awaiting Architect review**, not CODE ACCEPTED, not PRODUCTION ACCEPTED, not deployed. Branch `review/unified-conversation-compaction-a`. Canonical base `54b820d01d97c1ae44fba3d2d5831e1605e9c5d7`. Production remains `7610cc1c6ce24764fae73317a6925cdce7bd27b2`, Alembic **0041**. Migration **NONE**.
