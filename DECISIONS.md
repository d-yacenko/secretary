...
- **R4 local feedback / system assistant / lock screen:** accepted hardware trigger plays a native start earcon + short haptic with no network/TTS. Stop/process plays a distinct native stop earcon. On-screen mic uses local WAV assets. Volume Up double remains foreground-only. Background / lock-screen hands-free uses public `VoiceInteractionService` + user-selected Digital Assistant (`ROLE_ASSISTANT` where `RoleManager` exists). Lock-screen Voice is a separate per-user opt-in default off. Locked overlay is a minimal voice surface, not Inbox/history. External writes cannot approve/execute while keyguard is locked; exact reject remains allowed. Do not use Accessibility, root, adb remapping, private Samsung APIs, fake MediaSession volume hacks, or a wake word.
