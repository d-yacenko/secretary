...
Temporal Signals A — exact-time temporal hints + conservative calendar-first dedup + Week projection: **implemented / awaiting Architect review** on `review/temporal-signals-a` (not deployed; `temporal_signals_enabled` default **FALSE**; no historical backfill). Production base is Unified Week B-R7 at `99658a6f893483317dcecaf15d886cd1ed8d692b`. Review Alembic **`0035`**; production Alembic remains **`0034`** until Architect deploys. Encrypted Architect context untouched. Android **minSdk 23**. Proactive **OFF**, interval 60. Real-user `auto_label_enabled=true`. kalender **0.29.1** (exact). Availability / free-busy is **NOT** in this phase. Retained technical debt: Yandex attendee canonicalization / passive-sync metadata churn. Provider brand assets: deferred. Graph Advanced UX **NOT STARTED**.
