...
Temporal Signals A integration onto Cost Guard C lineage: **CODE ACCEPTED / awaiting deploy** at `6cf1c04fb87dc99b050b6d83d0995db5c2f9e876` on `review/temporal-signals-a-integration` (exact parent `bd6ebebc4d6475e45b8b7a91f3da253890d75ae2`; accepted Temporal source head `a9dc8c9c9e74dc4b8857fface89e803f493d21fd`). Alembic **0037 → 0036** (`user_settings.temporal_signals_enabled`). Lineage: 0035 Cost Guard B / `objects.embedding_signature`; 0036 Cost Guard C / `openai_daily_token_limit`; 0037 Temporal Signals / `temporal_signals_enabled`. Default `temporal_signals_enabled=false`. Temporal is not deployed and not enabled. No historical backfill. Cost Guards A+B+C remain **CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED**. Production application remains `7b580d955085ae7a5b3ae8c0a80b7f580cb938b1`. Production Alembic remains **0036 / 0036**.

OpenAI Cost Guard C / C-R1 — per-user daily OpenAI token hard cap: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `7b580d955085ae7a5b3ae8c0a80b7f580cb938b1` (C `27e0c097c43a81bbb9925f13a83734cc722a3201`; previous production `04f20f21d5460c9901d811ebd53863a370f85eda`). Alembic **0036 / 0036** (`user_settings.openai_daily_token_limit`). Final production fuse **NULL / disabled**. Flutter Account/Assistant show the fuse (from C; C-R1 did not change Flutter). User assistant model unchanged (`gpt-5.6-luna`). Source sync intervals unchanged. Proactive **OFF**, interval 60. Real-user `auto_label_enabled=true`. No dollar pricing, anomaly detection, or Cost Guard D.

Controlled production fuse test **PASSED**. Armed at U=293952 actual tokens today with temporary limit 293952. Two `/assistant/message` requests returned HTTP 429 `openai_daily_budget_exhausted` with zero new assistant `model_round` / `model_round_failed`. `tokens_used_today` did not increase while armed. One local-day warning notification; second request did not duplicate it. Source sync stayed healthy; no FAILED/job storm. Naturally blocked embed jobs parked until Moscow local-day reset with parked attempts 0 and zero embedding `model_round`. Clearing the limit released parked jobs without manual retry. Final `openai_daily_token_limit=NULL`, `exhausted=false`. Application SHA and Alembic unchanged.
