...
OpenAI Cost Guard A — correlation revision idempotency: **deployed / production verified / awaiting Architect post-deploy cost-rate acceptance** at `f20a68256b2a3061a4aaf9143893d7187a90a3d3` (previous production `99658a6f893483317dcecaf15d886cd1ed8d692b`). Temporal Signals A `a9dc8c9c9e74dc4b8857fface89e803f493d21fd` is **not merged / not deployed**. Alembic **`0034 / 0034`**. Flutter untouched. User assistant model unchanged (effective `gpt-5.6-luna`). Proactive **OFF**. Real-user `auto_label_enabled=true`. `temporal_signals_enabled` absent. Source sync jobs remain pending for all five connectors.

Post-deploy read-only window `2026-09-11T17:51:39Z`–`2026-09-11T18:24:45Z`: `background_correlation` traces 0, model calls 0, distinct objects 0 (vs 2026-09-09..11 baseline 23,757 calls / 23,569 yandex_calendar/event / 47 distinct Yandex events). Embedding traces in the same window: 448, all failed `credit_balance_exhausted`; `embed_object` jobs updated in-window: 448 failed, 0 pending. Correlate jobs still 26691 done / 11 failed / 0 pending/running. Observation only; no settings were changed from it.
