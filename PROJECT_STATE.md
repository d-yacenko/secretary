...
Telegram A: **CODE ACCEPTED / DEPLOYED / AWAITING TELEGRAM ACTIVATION** at `c5d288444cc5053e79c0942cdbc23af3eebbbb50` on `review/telegram-a`. Base `f0ee2b7045f4fc8459139945d5153e3528f5dfe0`. Telegram A initial `374d232279e2e3e1ac6e897ed8adfb87ec71333a`; docs awaiting-review `9268649877d272665c3a9a5534629b22ebb585ac`; A-R1 / CODE ACCEPTED / deployed `c5d288444cc5053e79c0942cdbc23af3eebbbb50`. Production Alembic **0039 / 0039**. Migration **0038 → 0039** (`telegram_accounts`, `telegram_link_states`). No `0040`. No backfill. Encrypted Architect context untouched.

Telegram A adds Bot API + Business/Secretary Mode inbound and `send_message` compose/reply for known private chats, with webhook ingress, explicit approval, and no native mute mirroring. Connector is **not activated**: `TELEGRAM_BOT_TOKEN` / `TELEGRAM_BOT_USERNAME` / `TELEGRAM_WEBHOOK_SECRET` / `TELEGRAM_WEBHOOK_URL` remain unset. No setWebhook. No identity link. No Telegram send. `GET /connections` telegram `configured=false`. `POST /telegram/link` HTTP 503. Webhook without secret HTTP 401. Zero Telegram Objects / jobs / polling.

Deployed from exact application SHA `c5d288444cc5053e79c0942cdbc23af3eebbbb50` (detached on VDS `/opt/secretary`). Procedure: `git checkout --detach c5d288444cc5053e79c0942cdbc23af3eebbbb50` then `cd infra && docker compose --env-file ../.env -f compose.yaml -f compose.deploy.yaml up -d --build`. Previous production `acf035a8665f0020209b460255156ac0bde5faf6`. Alembic before **0038 / 0038**, after **0039 / 0039**. API logs: `Running upgrade 0038 -> 0039, telegram_accounts and telegram_link_states`. `/health` healthy; api / worker / db healthy; FAILED `failed=1435` → `1435`. Settings unchanged: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`. Gmail and Google Calendar currently healthy after the user's post-Integration-A Google re-authorization (`last_success` 2026-09-13T13:08Z, `last_error` null; do not treat the old closure-doc OAuth message as current). Yandex Mail / Yandex Calendar / Mattermost last_success after deploy. Mattermost account `c54d71f6-cd7b-4044-8474-08884b9a9166` connected; `chat_message` **1321**; **0** duplicate `(provider,kind,external_id)`. Legacy flat Mattermost `send_message` plan `3e61e42e-f2dc-4260-9f33-1e786b9e716d` still loads as `provider=mattermost` without `route`; not re-executed; **zero** Telegram or Mattermost provider POST from this step. `/week` and `/availability` healthy; current week 7–13 Sep 2026 has HARD Google/Yandex events; next week has one temporal hint; `scheduled_work` / temporal hints remain non-blocking. Linux client rebuilt from the deployed SHA launched without Dart runtime exception (Inbox/Today/Week/`/me` 200). Android disconnect-button issue not repaired. No OpenAI/job storm attributable to Telegram (post-restart embeds were `yandex_calendar` only). Docs-only state commit is not deployed.
...
Integration A is **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `acf035a8665f0020209b460255156ac0bde5faf6`. Do not start the next phase from this ledger. Docs-only closure is not deployed.
