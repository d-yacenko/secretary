...
Temporal Signals A / A-R2: **implemented / awaiting Architect review** on `review/temporal-signals-a` (not deployed; A-R1 parent `a6cbe55869f7e04ea1a6dfb73eeb334dba7817dc`; A `3bcf0f94ff138df900615f130bacd04dc1afaa32`). Temporal Signals A itself is **NOT YET ARCHITECT-ACCEPTED**. `temporal_signals_enabled` default **FALSE**; no historical backfill. Production base is Unified Week B-R7 at `99658a6f893483317dcecaf15d886cd1ed8d692b`. Review Alembic **`0035`** (unchanged by A-R2); production Alembic remains **`0034`** until Architect deploys. Encrypted Architect context untouched. Android **minSdk 23**. Proactive **OFF**, interval 60. Real-user `auto_label_enabled=true`. kalender **0.29.1** (exact). Availability / free-busy is **NOT** in this phase. Week projects unresolved hints only while the opt-in is on; hints are not busy time. Retained technical debt: Yandex attendee canonicalization / passive-sync metadata churn. Provider brand assets: deferred. Graph Advanced UX **NOT STARTED**.

Unified Week A / A-R1 — Read-only unified calendar projection: **CODE ACCEPTED / DEPLOYED / HUMAN UX NOT ACCEPTED** at `eeb3ab4b49b3bdc1e2f6c2e5c6c90bddfc244d4a` (parent `e7d1b2086009840ca654a6b920e780220bdf2d58`; previous production `4945f19e6f6c134b435066d099d9b84d21612ce2`). Unified Week B through B-R7 — calendar time-grid on `kalender` 0.29.1; B-R7 production SHA `99658a6f893483317dcecaf15d886cd1ed8d692b`. Inbox Quick Actions — Swipe to Remove A / A-R1: **CLOSED / DEPLOYED / PRODUCTION ACCEPTED**. Design Quality Pass C: **CLOSED / DEPLOYED / PRODUCTION ACCEPTED**. Week A is a read-only Monday–Sunday projection of materialized Google Calendar + Yandex Calendar event Objects.
