...
Availability A-R1: **implemented / awaiting Architect review** at `e535adfb9857d2407fac06998a1a5a14c2b73502` on `review/availability-a` (parent `acc11e997670b15efbc39faee8c5c60d328683e8`; Availability A base `8d25c1ab6672744e85652ec917970d704553d5db`). Removed the 500-event availability query cap so every matching HARD event in the bounded window participates. Unknown-end HARD events (`due_at=NULL`) fail closed when `start_at < window_end` with no lower bound at `window_start`; Week/Today `event_overlaps_window` is unchanged. No Flutter change. No migration (`0039` not created); Alembic remains **0038 / 0038**; production application remains `76721b16650c49ce32b6405e8a001bdc6822bec4`; no deploy; zero Availability OpenAI calls; zero provider writes. Encrypted Architect context untouched. Production settings unchanged: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`, source sync intervals unchanged.

Availability A: **implemented / awaiting Architect review** at `acc11e997670b15efbc39faee8c5c60d328683e8` on `review/availability-a`, base `8d25c1ab6672744e85652ec917970d704553d5db`. Deterministic derived free/busy from materialized confirmed Google/Yandex calendar event Objects. `GET /availability` is read-only; no persistence of slots/busy/availability graph; no migration (`0039` not created); Alembic remains **0038 / 0038**; production application remains `76721b16650c49ce32b6405e8a001bdc6822bec4`; no deploy; zero Availability OpenAI calls; zero provider writes. Only HARD confirmed calendar commitments block availability; `scheduled_work` and `temporal_hints` do not. Unknown-end confirmed events fail closed. Encrypted Architect context untouched. Production settings unchanged: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`, source sync intervals unchanged.
...
Availability A-R1 is **implemented / awaiting Architect review** at `e535adfb9857d2407fac06998a1a5a14c2b73502` on `review/availability-a` (not deployed). Production remains Scheduled Work A at `76721b16650c49ce32b6405e8a001bdc6822bec4`. Do not mark Availability A PRODUCTION ACCEPTED.
