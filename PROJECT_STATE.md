...
Unified Conversation & Inbox Compaction A: **Architect R1 corrective implemented / awaiting Architect review, not CODE ACCEPTED, not PRODUCTION ACCEPTED, not deployed** on `review/unified-conversation-compaction-a`. Canonical base `54b820d01d97c1ae44fba3d2d5831e1605e9c5d7` (contains exact deployed application runtime tree `7610cc1c6ce24764fae73317a6925cdce7bd27b2`). Production remains `7610cc1c6ce24764fae73317a6925cdce7bd27b2`, Alembic **0041**. Migration **NONE**. Telegram transport/group acquisition deferred. Source acquisition unchanged (Gmail, Yandex Mail, Mattermost, Telegram, Teams). Conversation Stack is a derived presentation overlay (`conversation_groups`), not an Object and not a graph identity. False merge is worse than split. Summary is derived Representation `conversation_stack_summary`, fingerprint-bound. Review marker remains underlying Object/`feed_at` tuples; stacks split at the marker. Voice A code was not reopened. Graph Refinement A is not started. Voice B is not started.

Voice Assistant A factual carry-forward: canonical hands-free complete Inbox review was physically re-tested after R2; full review of the current 4-object window completed; `POST /inbox/review-marker/complete` returned 200; marker advanced to the frozen top; canonical Inbox marker completion gate is **PASS**. Wording variants such as «озвучь новые сообщения» may still lead to different tool behavior in edge cases; the user accepts current behavior for now. Real Teams inbound webhook proof remains deferred until a natural incoming Teams message is available. Voice A therefore remains **deployed / not formally PRODUCTION ACCEPTED**.

Voice Assistant A: **CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE** on `review/voice-assistant-a`. Accepted / exact deployed application SHA `7610cc1c6ce24764fae73317a6925cdce7bd27b2` (R2-R2; parent `acdff76dc86cfe5bd4ed82e1a5c619f480030afe`; previous production `b0c75eaa5e879ee108afe90f152a29914d28a2f2`; R2-R1 parent `44161b01b8b065204de0f5025fc55b96910f6058`; R2 application `244cdd28b81e7d8b33b6f78938fa062c03a82b3a`; R1 application `89c7e0bf9fc74dbfcc8a02f9a6795b4f3deae82e`). Previous production SHA `5ef2a2db117dc5a1c0ac54bad3d0e897ebcf6357`. **Final Production Corrective R2 is not CODE ACCEPTED** (`38d1e1a281f4f129cfc3d56924f193d85dd96c90`): Architect accepted marker receipt, client completion observability, and Teams v1.0 change-notification architecture, but rejected using `chat.lastUpdatedDateTime` as message freshness. **R2-R1 direction is Architect-accepted** (`30fb7553ec343764dfcd8eb69b8316f9431b0b92`) but that SHA was **not CODE ACCEPTED** due to permanent Teams notification `enqueue_once` dedupe. **Final Production Corrective R2 is CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE**, not PRODUCTION ACCEPTED, at exact runtime application SHA `7610cc1c6ce24764fae73317a6925cdce7bd27b2` (parent `acdff76dc86cfe5bd4ed82e1a5c619f480030afe`; previous production `b0c75eaa5e879ee108afe90f152a29914d28a2f2`). Alembic **0040 → 0041**. Teams webhook `https://web-itx.duckdns.org/secretary/webhooks/teams`; reconciliation interval **900s**. Graph v1.0 subscription **201** `active`. Debug APK sha256 `e6933829d12ab3ea7f4793c92c00e5a6394c339aca24bbe618b444e1e18305c9`, aapt `sdkVersion:'23'`, installed on SM_T355. Production diagnosis (read-only): post-deploy review turn `192d1c3d` called `list_inbox_since_review_marker` with `purpose=review` `limit=20` no cursor; follow-up `dbd4e2b2` was `inspect` `limit=1`; marker `c6260075…` / `2026-09-15 09:56:38+00` `updated_at=11:42:50Z` unchanged; **zero** `POST /inbox/review-marker/complete`. Code: utterance-forced review/inspect bind; receipt from domain snapshot not char-truncated model view; client completion diagnostics remain fail-closed. Teams Graph v1.0 probe on the connected tenant **HTTP 201** for `/users/{microsoft-user-id}/chats/getAllMessages` (`includeResourceData=false`), probe subscription deleted **204**. Alembic **0041** (`teams_subscriptions`) is in this corrective tree only; production Alembic remains **0040 / 0040**. **R3 is not CODE ACCEPTED.** **R4 is not CODE ACCEPTED.** **R4-R1 is not CODE ACCEPTED** (`e097069dad8bcb0066e531a63f5f668e786e62ab`). **R4-R2 is not CODE ACCEPTED** (`f8c9a256af9d07b77b088e94fc1110e69479343f`). **R4-R3 is not CODE ACCEPTED**; capture corrective application SHA `d121cf90e351612275663c6845a76e9dd80ed587` (Architect: Android foreground hardware / repeated STT / STT→Assistant→TTS / ready-tone / calendar questions physically PASS). **R4-R4 (complete Inbox review snapshots + completion-based review marker) is included in the combined CODE ACCEPTED / DEPLOYED tree** (`dc3568d2cb2e4808f4a736933b0ce615553a11eb`). **R4-R4-R1 (chronological Inbox review order) is implemented / awaiting Architect review, not CODE ACCEPTED, not deployed**, application SHA `0ab2cb6f1ae2a373f8b719314777824434c9adda` (parent `c4bcd66020d747668eb99ea03ea7613814e4b8d8`). Frozen `purpose=review` paginates oldest→newest inside the frozen window after discovering `snapshot_top` as the newest eligible item; `inspect` and Inbox UI remain newest-first. Cursor v=2 binds traversal `dir`. Completion still advances to frozen newest top after verified playback. **R4-R4-R2 (on-screen microphone strictly silent-output) is Architect scope-accepted, not CODE ACCEPTED, not deployed**, application SHA `276fe709527a7d300660cbe444ae40ee520e7e05` (parent `09f58b9ebf261df728352faf239ff2759cf12031`). Client-only: `screenMic` never auto-speaks; local policy is `handsFreeEnabled` / `never`; legacy `all_voice_input` migrates to hands-free-enabled. Backend R4-R4-R1 unchanged. No provider read-state mutation. R4-R3 media cues no longer overlap microphone capture. **Lock-screen Launcher A / R1 functional completion + accepted UI polish is implemented / awaiting Architect review, not CODE ACCEPTED, not deployed**, application SHA `319a99e5c2d55750fd9949f8a31fd137c45b703b` (UI polish parent `b99bbad50c7d0d8a8edbef8d05781a2c7fb9de23`; previous docs tip `489eb9c1300fac11e6766a9cc155d129cb9cfd2a`). **Final Inbox/Source Corrective is CODE ACCEPTED / DEPLOYED as part of `b0c75eaa5e879ee108afe90f152a29914d28a2f2`**, application SHA `f50b56c6d874148963479830ad25af5b65522c20` (parent `319a99e5c2d55750fd9949f8a31fd137c45b703b`). Production diagnosis was read-only against runtime `5ef2a2db117dc5a1c0ac54bad3d0e897ebcf6357`: hung `sync_yandex_mail` (`locked_at=2026-09-14T11:37:18.648892Z`) blocked the single general source-sync lane; Gmail `last_success_at=2026-09-14T11:37:15.389530Z` so later Gmail list never re-ran (not proven noise-label exclusion); Teams connected `2026-09-15T04:59:55.120706Z` with `sync_start_at` the same, 0 Teams objects, `sync_teams` never claimed so Graph `list_chats` was never reached. Code: one worker lane per recurring source-sync job type plus Yandex IMAP 30s socket timeout; complete-enumeration utterances (`перечисли все новые сообщения` and equivalents) are `purpose=review`; count/peek remains `inspect`. Marker observation on the new client against the old production backend is not a client-bug proof (R4-R4 receipts are not deployed). No lock icon. No visual-identity orb redesign. Physical marker gate A–J is the remaining user-manual acceptance after this combined deploy. Production worker was recreated with the accepted tree (not a same-SHA restart of the hung worker). **Final Client Shortcut Completion is CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE**, application SHA `b0c75eaa5e879ee108afe90f152a29914d28a2f2` (parent docs tip `afaab5859b57c5a34c9dcb5eb14f92e790dcbae0`; accepted backend/source base `f50b56c6d874148963479830ad25af5b65522c20`). Android-only `Icons.directions_car` sits next to the existing Account action (narrow AppBar left of `shell_account_button`; wide NavigationRail trailing above it); tooltip «Режим вождения». Linux/desktop has no car shortcut. Enabled `lock_screen_voice_enabled.<userId>` opens the existing Driving launcher once; disabled shows compact confirmation, Cancel mutates nothing, «Включить и открыть» persists through existing `setLockScreenVoiceEnabled` then the same launcher once. Account Driving Mode section remains. No second launcher, no `ROLE_ASSISTANT`, no orb redesign. Backend/source-sync/review-marker unchanged. Debug APK sha256 `b127fb5b061d6910c037c43576bf225a8a612f2bac30b79b0d1e0700a5bb3729`; aapt `sdkVersion:'23'`. Client-only: Google/Gemini stays the Android default assistant; Account no longer requests `ROLE_ASSISTANT`. Explicit driving launcher reuses `VoiceSessionActivity` (`showWhenLocked` / `turnScreenOn`, no `FLAG_KEEP_SCREEN_ON`) in idle `launcher` mode; `VoiceInvocationSource.lockScreenLauncher` is hands-free and frozen per turn. Opening Driving Mode while authenticated and unlocked arms an in-memory session nonce (not prefs/server; cleared on close/auth-loss/process death). Canonical `mayVoiceApproveLockedPendingPlan` lets exact voice «Да» execute a fully narrated `send_email`/`send_message` plan while keyguard stays locked; visual approve and all other locked writes remain fail-closed. Voice is not biometric identity. `VoiceSessionApp` follows system light/dark (`ThemeMode.system`); the ordinary Secretary app remains light-only. Driving Account section sits immediately after Profile on Android. Local UI text scale is 50–130% in 5% steps. Inbox review marker uses `Color(0xFFFF4D2E)`. Linux/desktop review-rail click persists the marker; Swipe-to-Remove stays Android/iOS. If the OS kills the activity after sleep, resurrection is not required; the user reopens driving mode. No AccessibilityService, no always-listening mic, no notification/FGS fallback. Locked overlay never shows sign-in forms. No DB migration. Production was not mutated. R1 PASS. R2 marker architecture PASS (auto-move rule refined in R4-R4). R2-R1 reference hygiene PASS. Linux user physical launch + Voice PASS (prior). Android Inbox-review physical smoke remains user-run after this APK. Not PRODUCTION ACCEPTED. Encrypted Architect context untouched. Alembic **0041 / 0041** (Voice A Final Production Corrective R2: `teams_subscriptions`). Detached VDS deploy of the accepted application SHA rebuilt `api`/`worker` only; PostgreSQL volume `db_data` was not recreated. Pre-deploy backup `/opt/secretary/backups/pre-voice-a-20260914T075123Z-eb3af922.dump`. Combined Voice A deploy backup `/opt/secretary/backups/pre-voice-a-combined-20260915T140527Z-5ef2a2db.dump`. R2 deploy backup `/opt/secretary/backups/pre-voice-a-r2-20260915T180737Z-b0c75eaa.dump`. Documented TTS/STT defaults appended to production `.env` (`OPENAI_TTS_MODEL=gpt-4o-mini-tts`, `OPENAI_TTS_VOICE=alloy`, `OPENAI_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe`); existing per-user OpenAI credential path unchanged. FAILED `1435` → `1435`. Internal and public `/health` 200. Authenticated `/assistant/speech` smoke: HTTP 200, `audio/mpeg`, 16896 bytes, objects/representations unchanged. Android debug APK minSdk **23**. Linux plugin RUNPATH rewritten to `$ORIGIN` in R4-R1; this host still lacks `gstreamer-devel` (classification C). No Voice B. Inbox Conversation Compaction A is implemented on `review/unified-conversation-compaction-a` (awaiting Architect review, not deployed). Graph Refinement A is not started.
...
Inbox consecutive swipe-delete: `ListView` children and `InboxSwipeToRemove` had no object identity, so after deleting A the State of A (`_deleteInFlight=true`) was reused for B and `confirmDismiss` returned false (spring-back). Rows are keyed by object id; `didUpdateWidget` resets `_armed` / `_deleteInFlight` if the id ever changes.

Phone Week: compact `< 600dp` used `MultiDayViewConfiguration.singleDay`, so the Week tab showed one giant day. Compact now uses kalender 0.29.1 `MultiDayViewConfiguration.custom(numberOfDays: 3)` with a display range aligned to the 3-day page. Current-week initial page contains today (Mon → Mon/Tue/Wed; midweek previous/current/next; Sunday → Fri/Sat/Sun). Non-current week opens Mon/Tue/Wed. Horizontal paging remains; week-boundary `onPageChanged` still requests Monday `WeekOut`. Wide/tablet >= 600dp keep `MultiDayViewConfiguration.week` (7 days). No custom calendar engine.

Teams A + Communication Action Plan Integrity: **CODE ACCEPTED / DEPLOYED / PRODUCTION ACTIVATION DEFERRED — EXTERNAL ENTRA ADMIN CONSENT REQUIRED** on `review/teams-a`. Not CLOSED. Not PRODUCTION ACCEPTED. Canonical base `fd5d1497215e4c72033274908b69aebe42e175a9` (`review/telegram-a`, “Close Telegram A after production activation”). CODE ACCEPTED / exact deployed application SHA `eb3af922f804e6faf1d895fd14c0973981ce515e` (parent `fb980a886e406e816570603391f8319988271ab9`; accepted R2 `6fee41b57b8421699cefc1443c2e8e639f1214fe`; accepted R1 `49396cad686cc4631254ec389657b7a52d0dcf8f`; prior application `8a1a6fe8013f16cce9c85a9143217ef508cf7ee2`). Previous production SHA `c5d288444cc5053e79c0942cdbc23af3eebbbb50`. Alembic **0039 / 0039** → **0040 / 0040** (`0040_teams_accounts.py`: `teams_accounts`, `teams_oauth_states`). No `0041`. No backfill. Encrypted Architect context untouched. This ledger transition is docs-only: no application code change, no deploy, no production configuration change.

Deployed detached on VDS `/opt/secretary`: `git checkout --detach eb3af922f804e6faf1d895fd14c0973981ce515e` then `cd infra && docker compose --env-file ../.env -f compose.yaml -f compose.deploy.yaml up -d --build`. Runtime application SHA remains `eb3af922f804e6faf1d895fd14c0973981ce515e`. Alembic remains **0040 / 0040**. `/health` 200; api / worker Up, db healthy. FAILED `1435` → `1435` (pre-existing; not repaired). Settings unchanged: `assistant_model` NULL, `openai_daily_token_limit=1500000`, `temporal_signals_enabled=true`, `proactive_enabled=false`, `proactive_interval_minutes=60`, `auto_label_enabled=true`. Source prefs unchanged (interval 60s): gmail history 60, google_calendar, mattermost, yandex_calendar, yandex_mail history 30. Recurring Gmail / Google Calendar / Mattermost / Yandex Calendar / Yandex Mail continued after deploy. Existing accounts/objects were not mutated by Teams (`mattermost_accounts=1`, `telegram_accounts=1`). `teams_accounts=0`, no Teams token, no Teams send, `sync_teams` jobs=0, Teams objects=0. No unintended external writes. Docs-only commits are not deployed.

Microsoft runtime configuration is present: `teams_is_configured()` true; `GET /connections` Teams `configured=true`, `connected=false`, `reconnect_required=false`. Redirect `https://web-itx.duckdns.org/secretary/auth/teams/callback`. The user initiated real Microsoft OAuth; Microsoft recognized the Secretary Entra application and the work/school account. OAuth was blocked by the organization's Microsoft Entra tenant policy with **Need admin approval**. This is an **EXTERNAL TENANT ADMIN-CONSENT BLOCKER**, not a Secretary code defect. Required configured Graph delegated permissions: `User.Read`, `Chat.Read`, `ChatMessage.Send`, plus OIDC `openid` / `profile` / `offline_access`. The accidental `ChatMessage.Read` permission was removed from Entra. Live inbound Teams object / first-turn Pending Action Plan / no-write-before-approval were not reached. Real Secretary-originated Teams send: **not executed, not claimed as production-tested**. This external blocker **no longer blocks subsequent Secretary development phases**. Teams production activation may be resumed later from the accepted/deployed SHA `eb3af922f804e6faf1d895fd14c0973981ce515e` after tenant admin consent becomes available.

Telegram A: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at deployed application SHA `c5d288444cc5053e79c0942cdbc23af3eebbbb50` on `review/telegram-a`. Base `f0ee2b7045f4fc8459139945d5153e3528f5dfe0`. Telegram A initial `374d232279e2e3e1ac6e897ed8adfb87ec71333a`; docs awaiting-review `9268649877d272665c3a9a5534629b22ebb585ac`; A-R1 / CODE ACCEPTED / deployed `c5d288444cc5053e79c0942cdbc23af3eebbbb50`; docs deployed-awaiting-activation `7e58d4286196bcf1d6d4ef48aefd803d50697347`. Production Alembic **0039 / 0039**. Migration **0038 → 0039** (`telegram_accounts`, `telegram_link_states`). No backfill. Encrypted Architect context untouched.

Telegram A adds Bot API + Business/Secretary Mode inbound and `send_message` compose/reply for known private chats, with webhook ingress, explicit approval, and no native mute mirroring. Production activation proven: webhook via accepted CLI; identity link; Business/Secretary connection; `can_reply`; live inbound Inbox path; Inbox filtering of user-originated outbound Telegram Objects. Bot username `personal_secretary_assistant_bot`. Webhook URL `https://web-itx.duckdns.org/secretary/integrations/telegram/webhook`. `getMe` username matched; `can_connect_to_business=true`; `allowed_updates` `message`, `business_connection`, `business_message`, `edited_business_message`, `deleted_business_messages`; no pending webhook error at activation. `GET /connections` telegram `configured=true`, `identity_linked=true`, `business_connected=true`, `can_reply=true`. `telegram_accounts=1`; link state consumed; `/start` did not materialize as `chat_message`. Live inbound Object `402c3b51-6f58-43a4-9d7e-236024b5cce5`. Observed live Objects in the selected test chat: 1 inbound + 3 user-originated outbound; outbound stored as conversation context, not Inbox arrivals. No history backfill. No mark-read. No Telegram temporal extraction. Assistant/provider outbound `send_message` remains CODE ACCEPTED and test-covered. Real Secretary-originated Telegram `send_message` production write: **DEFERRED BY USER CHOICE / NON-BLOCKING** (not executed, not claimed as tested). Real Mattermost `send_message` production write: also deferred; no unsolicited provider write.

Post-closure manual production evidence (2026-09-13, production HEAD still `c5d288444cc5053e79c0942cdbc23af3eebbbb50`, Alembic **0039 / 0039**): a real Mattermost message was successfully sent through Secretary; a real Telegram reply was successfully sent through Secretary. Observed Telegram UX defect: with the exact `chat_message` already in Assistant UI context and an explicit reply request, the first turns never called `send_message` (diagnosis **A**), the model answered in prose («Подтвердите отправку») with no Pending Action Plan card, and only a later explicit «Отправляй» turn staged a real plan that approved and sent. Teams A includes the communication Action Plan Integrity corrective. This does not reopen or redesign Telegram A.

Deployed from exact application SHA `c5d288444cc5053e79c0942cdbc23af3eebbbb50` (detached on VDS `/opt/secretary`). Procedure: `git checkout --detach c5d288444cc5053e79c0942cdbc23af3eebbbb50` then `cd infra && docker compose --env-file ../.env -f compose.yaml -f compose.deploy.yaml up -d --build`. Previous production `acf035a8665f0020209b460255156ac0bde5faf6`. Alembic before **0038 / 0038**, after **0039 / 0039**. Closure pulse: production HEAD still `c5d288444cc5053e79c0942cdbc23af3eebbbb50`; Alembic **0039 / 0039**; `/health` 200; api / worker Up, db healthy; FAILED `failed=1435` → `1435`. Gmail / Google Calendar / Yandex Mail / Yandex Calendar / Mattermost `scheduled`, `last_error` null, `last_success` 2026-09-13T14:17Z. Telegram jobs bounded (`embed_object` 4 done, `auto_label_object` 4 done, temporal 0). Recent embeds otherwise existing source sync (`yandex_calendar` dominant). No OpenAI/job storm attributable to Telegram. Docs-only closure commit is not deployed.

Integration A: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `acf035a8665f0020209b460255156ac0bde5faf6` on `review/unified-communications-a-integration`. Production Alembic at that close was **0038 / 0038**. Migration **NONE** at that close. No backfill. Docs-only closure is not deployed.
...
Alembic before and after **0038 / 0038**. `/health` healthy; api / worker / db healthy; FAILED `failed=1435` → `1435`. Settings unchanged: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`, source sync intervals unchanged (60s). Mattermost / Yandex Mail / Yandex Calendar last_success after deploy. Gmail / Google Calendar remain pre-existing `failed to refresh access token` (last_success 2026-09-13T06:21Z, before deploy; not repaired). Mattermost account `c54d71f6-cd7b-4044-8474-08884b9a9166` connected; `chat_message` **1320**; **0** duplicate `(provider,kind,external_id)`; 13 tombstoned. `send_message` staged plan `3e61e42e-f2dc-4260-9f33-1e786b9e716d` from object `86e0b94c-0fb2-4f0f-9719-30c3f7562efa`; provider `mattermost`; `pending_post_id` `secretary:848ba8f0c3a54592ad659c0c5e51452b`; `operation_id` persisted; rejected via normal flow; **zero** provider POST; **0** `send_message` attempts. Real Mattermost send: **deferred pending explicit safe target**. Linux client from the deployed SHA launched without runtime exception. Week hourly-grid runtime healthy (HARD events, today tint, now-line, event tap → Object Detail). Natural temporal pill visible on week 14–20 Sep (`Встреча 14 сентября`); current week 7–13 had no `temporal_hints` in `GET /week`. Account disconnect dialog on PC opened and was cancelled; production client was **not** disconnected. Android tablet: disconnect button visible but tap does nothing (observed; not repaired). `/week` and `/availability` healthy; HARD Google/Yandex remain blocking; `scheduled_work` / temporal hints remain non-blocking. No OpenAI/job storm. Encrypted Architect context untouched.

Design Quality Pass D: **CLOSED as the Integration A client tree** (`9b433013c9651a38d1d4f3127e3d44189982d6b5` identical in the deployed SHA).

Unified Communications A: CODE ACCEPTED at `a58c77ad394995067b682157275c5c9026382484`. Integrated backend tree is exactly the accepted backend tree and is included in production `acf035a8665f0020209b460255156ac0bde5faf6`.
...
Availability is a deterministic derived read model calculated on demand from already materialized confirmed Google/Yandex calendar events. Only HARD confirmed calendar commitments block availability. Week layers: confirmed calendar events HARD, `scheduled_work` SOFT, `temporal_hints` TENTATIVE. `scheduled_work` and `temporal_hints` do not block availability. Availability persists no free slots or busy intervals, creates no Object/Edge/table, performs no provider fetch/write, performs no OpenAI/model call, and creates no background jobs. `GET /availability` is authenticated and read-only. Window <= 7 days. No authoritative event-count cap. Known-end HARD overlap: `start_at < window_end` AND `due_at > window_start`. Unknown-end HARD: `due_at IS NULL` AND `start_at < window_end`; fail-closed (`availability_complete=false`, id in `unknown_end_event_ids`, `free_intervals=[]`). Week/Today unknown-end display semantics unchanged. Known HARD intervals clip to the window; overlapping and directly adjacent HARD intervals merge; free intervals are the complement of merged HARD and respect `min_duration_minutes`. Timezone arithmetic uses aware instants / UTC.

Production `76721b16650c49ce32b6405e8a001bdc6822bec4` → `e535adfb9857d2407fac06998a1a5a14c2b73502`. Alembic remained **0038 / 0038**. `/health` healthy; api / worker / db healthy; FAILED `1435` → `1435`. Settings unchanged: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`, source sync intervals unchanged.

Main smoke timezone `Europe/Moscow`, window 2026-09-12 09:00–18:00 Europe/Moscow. HARD: `53584b6f-817c-421d-8616-0a1f5d1a576a` google_calendar 09:50–11:20 MSK; `3a7e38ef-9942-41eb-88c0-0dc8dedd6974` yandex_calendar 13:30–14:00 MSK. Busy matched those events; free complement 09:00–09:50, 11:20–13:30, 14:00–18:00, all >= 30 minutes. Independent DB-vs-API matched. Clip smoke confirmed. Natural overlap/merge 17 Sep 2026 18:00–20:00 MSK: Google all-day plus timed Google event → one merged HARD busy covering the window with both event ids. Temporal hint `aafcdfde-ad0e-4b38-bcd4-e546a43afff8` did not block. No natural production unknown-end case; accepted A-R1 regressions cover unknown-end inside/before the window and exclusion at/after `window_end`. No natural `scheduled_work` overlap; accepted regression proves it does not block. Empty-window smoke returned one full free interval. Deployed code has no 500-event cap; >500 correctness is the accepted A-R1 regression. Availability caused zero provider writes, zero FreeBusy calls, zero Availability jobs, zero Availability OpenAI calls, zero Object/Edge mutations. Temporal Signals, Scheduled Work / Week, and source sync remained healthy. Flutter production runtime showed no discovered runtime error. Wayland automation could not click/capture the Flutter GTK window (`xdotool` cannot see it; `flutter screenshot` unsupported on Linux) — automation/capture limitation, not a product defect. Encrypted Architect context untouched.

Scheduled Work A: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `76721b16650c49ce32b6405e8a001bdc6822bec4` on `review/scheduled-work-a`. Production Alembic **0038 / 0038**. Migration **0038 → 0037** (`objects.planned_start_at` / `objects.planned_end_at` TIMESTAMPTZ NULL; both NULL or `kind=task` and both set with `planned_end_at > planned_start_at`). No backfill. Scheduled work is an explicit Object planned-execution interval, not `due_at`, not a calendar event, not hard busy, and does not write Google/Yandex calendars. Week layers: events HARD, `scheduled_work` SOFT, `temporal_hints` TENTATIVE. Week `scheduled_work` fail-closed to `open` / `in_progress` / `done` (NULL, legacy `completed`, cancelled, archived, deleted, unknown excluded). Upgrade **0037 → 0038** succeeded; `/health` healthy; existing tasks stayed unscheduled; FAILED count did not increase. Smoke `ad24f373-700e-49d0-b5e0-31567e9e5bfd`: capture left interval NULL; Object PATCH set 2026-09-12 20:00–20:45 Europe/Moscow; `due_at` stayed NULL; same id in `scheduled_work` only; open/done visible, cancelled hidden; clear returned both fields to NULL and removed it from Week; normal soft-delete. Zero Scheduled Work calendar writes or OpenAI calls. Temporal Signals remained healthy. Cost Guards unchanged. Production settings: `temporal_signals_enabled=true`, `openai_daily_token_limit=1500000`, assistant model `gpt-5.6-luna`, proactive **false** / interval 60, `auto_label_enabled=true`, source sync intervals unchanged. Encrypted Architect context untouched. Docs-only closure is not deployed.

Temporal Signals A: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `6cf1c04fb87dc99b050b6d83d0995db5c2f9e876` on `review/temporal-signals-a-integration`. Production Alembic **0037 / 0037**. Migration **0037 → 0036** (`user_settings.temporal_signals_enabled`). Real-user `temporal_signals_enabled=true`. No historical backfill. Cost Guards A+B+C remain intact / production accepted. Current real-user `openai_daily_token_limit=1500000`. Assistant model `gpt-5.6-luna`. Proactive **false** / interval 60. `auto_label_enabled=true`. Source sync intervals unchanged.

Production acceptance: a new real Gmail arrived through normal source sync; exact-time signal for 14 Sep 2026 15:30 Europe/Moscow; result `hint_created`; `due_at=NULL`; `participation=expected`; source evidence signature/version bound. Week exposes the object only through `temporal_hints`, not events/busy. One paid Temporal extraction for that source revision; no duplicate extraction for unchanged `source_signature` over ~22 min; no Temporal hot loop; no new FAILED jobs; source sync healthy; Cost Guard not exhausted. No manual enqueue / synthetic job / backfill.

OpenAI Cost Guard C / C-R1 — per-user daily OpenAI token hard cap: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** (included in current production `6cf1c04fb87dc99b050b6d83d0995db5c2f9e876`; C/C-R1 application SHA `7b580d955085ae7a5b3ae8c0a80b7f580cb938b1`; C `27e0c097c43a81bbb9925f13a83734cc722a3201`). Alembic introduced **0036** (`user_settings.openai_daily_token_limit`). Current real-user configured limit is **1500000**. The Cost Guard C fuse test historically ended with `openai_daily_token_limit=NULL` / disabled; that NULL is test evidence, not the current production value. Flutter Account/Assistant show the fuse (from C; C-R1 did not change Flutter). User assistant model unchanged (`gpt-5.6-luna`). Source sync intervals unchanged. Proactive **OFF**, interval 60. Real-user `auto_label_enabled=true`. No dollar pricing, anomaly detection, or Cost Guard D.

Controlled production fuse test **PASSED** (historical). Armed at U=293952 actual tokens today with temporary limit 293952. Two `/assistant/message` requests returned HTTP 429 `openai_daily_budget_exhausted` with zero new assistant `model_round` / `model_round_failed`. `tokens_used_today` did not increase while armed. One local-day warning notification; second request did not duplicate it. Source sync stayed healthy; no FAILED/job storm. Naturally blocked embed jobs parked until Moscow local-day reset with parked attempts 0 and zero embedding `model_round`. Clearing the limit released parked jobs without manual retry. Test-end `openai_daily_token_limit=NULL`, `exhausted=false`. That NULL was the fuse-test endpoint only; current real-user configured value is **1500000**.

OpenAI Cost Guard C-R1 — in-flight usage + transcription accounting + timezone reset: included in production SHA `7b580d955085ae7a5b3ae8c0a80b7f580cb938b1` (parent `27e0c097c43a81bbb9925f13a83734cc722a3201`). Same-trace sequential paid calls include uncommitted actual tokens. Transcription records SDK token usage when present. Timezone settings changes release budget-parked jobs.

OpenAI Cost Guard B — embedding revision idempotency: **CLOSED / CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `04f20f21d5460c9901d811ebd53863a370f85eda` (previous production `f20a68256b2a3061a4aaf9143893d7187a90a3d3`). Alembic **0035 / 0035** (`objects.embedding_signature`). Cost Guard A remains **CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED**. Flutter untouched. User assistant model unchanged (`gpt-5.6-luna`). Source sync intervals unchanged. No embedding backfill.
...
OpenAI Cost Guard A — correlation revision idempotency: **CODE ACCEPTED / DEPLOYED / PRODUCTION ACCEPTED** at `f20a68256b2a3061a4aaf9143893d7187a90a3d3` (parent `99658a6f893483317dcecaf15d886cd1ed8d692b`). Job-level `correlation_input_signature` following the auto-label pattern. Background correlation uses LOW/LOW reasoning independent of interactive Assistant settings. Permanent OpenAI quota exhaustion does not burn three retries on non-recurring AI jobs.

Unified Week A / A-R1 — Read-only unified calendar projection: **CODE ACCEPTED / DEPLOYED / HUMAN UX NOT ACCEPTED** at `eeb3ab4b49b3bdc1e2f6c2e5c6c90bddfc244d4a` (parent `e7d1b2086009840ca654a6b920e780220bdf2d58`; previous production `4945f19e6f6c134b435066d099d9b84d21612ce2`). Unified Week B — calendar time-grid on `kalender` 0.29.1. Unified Week B-R1 — overlap layout, bookmark markers, copy contract at `e7d9e5084707137384a3b21f40a2d4d1d34708f0`. Unified Week B-R2 — dense overlap, depth tones, wide typography at `d4627c8749f4dd786632a38a28e8feb7ec7d453f`. Unified Week B-R3 — tighter overlap, today-column anchor, provider vs temporal item type at `915f7d56aca07d86d3e7201344120d5ef5657a27`. Unified Week B-R4 — final perceptual tuning at `cb99f40cdf50f7e88ce108d857be2de36c5b2ae2`. Unified Week B-R5 — high-contrast type glyph + passive Week snapshot refresh at `6b7e0ed575b8d18f616a0752f689784030180b21`. Unified Week B-R6 — desktop viewport calmness, Today date badge, task-button microcopy at `5e05f7ec25b668d727d69425ef1b84f5fefa0476`. Unified Week B-R7 — full Today header label + morning desktop viewport: **implemented / awaiting Architect review** on `review/unified-week-b` (not deployed). Role-boundary hygiene is included in this production advance. Inbox Quick Actions — Swipe to Remove A / A-R1: **CLOSED / DEPLOYED / PRODUCTION ACCEPTED**. Design Quality Pass C: **CLOSED / DEPLOYED / PRODUCTION ACCEPTED**. Week A is a read-only Monday–Sunday projection of materialized Google Calendar + Yandex Calendar event Objects. Availability / free-busy is **NOT** in this phase. Encrypted Architect context untouched. Alembic **`0034 / 0034`**. Android **minSdk 23**. Proactive **OFF**, interval 60. Real-user `auto_label_enabled=true`. Retained technical debt: Yandex attendee canonicalization / passive-sync metadata churn. Provider brand assets: deferred. Graph Advanced UX **NOT STARTED**.
...
PHASE 28B-C — Per-User Request-Time Graph Embeddings: **accepted / closed** (`485e24942fb778e1952153394dccac552f9e88c3`).
...
PHASE 28C-B2-C1-A — Yandex Mail Bounded History Foundation: **accepted** at `cdbd667616cebcab9602be8ab24f31fd0d8f1514`.

PHASE 28C-B2-C1-B — Yandex Mail Bounded History Runtime: **accepted** at `e56526ac2e16b681ac1cffaf94cf42a1804bcab0`.
...
PHASE 28C-B2-C2-B — Yandex Calendar History Preference Runtime: **accepted** at `fdaf34cbd3764fb8f03c5d19666e15a0401439bb`.
...
Format Parity — Pass A (backend extraction kernel + ODF + adaptive datasets): **implemented** at `5c4ca2d013946c3b904624e766b389af66021569`; Yandex storage-host corrective at `4fef52424397235d65ee8a7f0aceb25549527e6f`. **Production verification complete**; **awaiting final architect closure acceptance** on `review/format-parity-a`. Post-`4fef524` Yandex ODS smoke **PASS** (`*.storage.yandex.net` defect closed). Google Drive disposable ODT E2E **BLOCKED** (`drive.readonly`, not expanded for testing). No bulk stale-version backfill **PASS**.
...
Voice Assistant A combined tree is **CODE ACCEPTED / DEPLOYED / AWAITING FINAL USER MANUAL ACCEPTANCE** at application SHA `b0c75eaa5e879ee108afe90f152a29914d28a2f2` on `review/voice-assistant-a` (previous production `5ef2a2db117dc5a1c0ac54bad3d0e897ebcf6357`). Alembic **0040 / 0040**. Backup `/opt/secretary/backups/pre-voice-a-combined-20260915T140527Z-5ef2a2db.dump`. Gmail recovered past the 2026-09-14T11:30:22Z Object boundary. Teams Graph list_chats reached; one group `chat_message` materialized. User physical Inbox/marker gate A–J remains. **Final Production Corrective R2 is implemented / awaiting Architect review and is not deployed.** Not PRODUCTION ACCEPTED. Do not start Voice B.
