...
Teams A + Communication Action Plan Integrity: **CODE ACCEPTED / DEPLOYED / TEAMS ACTIVATION BLOCKED** on `review/teams-a`. Not CLOSED. Not PRODUCTION ACCEPTED. Canonical base `fd5d1497215e4c72033274908b69aebe42e175a9` (`review/telegram-a`, “Close Telegram A after production activation”). CODE ACCEPTED / exact deployed application SHA `eb3af922f804e6faf1d895fd14c0973981ce515e` (parent `fb980a886e406e816570603391f8319988271ab9`; accepted R2 `6fee41b57b8421699cefc1443c2e8e639f1214fe`; accepted R1 `49396cad686cc4631254ec389657b7a52d0dcf8f`; prior application `8a1a6fe8013f16cce9c85a9143217ef508cf7ee2`). Docs tip `511a3f36a20fc9bc91170d010d33cee486fd2c0b` was not deployed. Previous production SHA `c5d288444cc5053e79c0942cdbc23af3eebbbb50`. Alembic **0039 / 0039** → **0040 / 0040** (`0040_teams_accounts.py`: `teams_accounts`, `teams_oauth_states`). No `0041`. No backfill. Encrypted Architect context untouched.

Deployed detached on VDS `/opt/secretary`: `git checkout --detach eb3af922f804e6faf1d895fd14c0973981ce515e` then `cd infra && docker compose --env-file ../.env -f compose.yaml -f compose.deploy.yaml up -d --build`. Runtime HEAD after start is `eb3af922f804e6faf1d895fd14c0973981ce515e`. `/health` 200; api / worker Up, db healthy. FAILED `1435` → `1435` (pre-existing; not repaired). Settings unchanged: `assistant_model` NULL, `openai_daily_token_limit=1500000`, `temporal_signals_enabled=true`, `proactive_enabled=false`, `proactive_interval_minutes=60`, `auto_label_enabled=true`. Source prefs unchanged (interval 60s): gmail history 60, google_calendar, mattermost, yandex_calendar, yandex_mail history 30. Recurring `sync_google_calendar` / `sync_google_gmail` / `sync_mattermost` / `sync_yandex_calendar` / `sync_yandex_mail` remain `pending` with empty `last_error` and `last_success` after deploy (~2026-09-13T18:11Z). Existing accounts/objects unchanged (`mattermost_accounts=1`, `telegram_accounts=1`; Gmail/Google/Yandex/Mattermost/Telegram object counts unchanged). `teams_accounts=0`, `teams_oauth_states=0`, `sync_teams` jobs=0, Teams objects=0. Runtime `teams_is_configured()` false. No Teams job/OpenAI storm. Recent embeds remain pre-existing `yandex_calendar` signature churn. No unintended external writes. Google/Yandex/Mattermost/Telegram credentials and production `.env` not rewritten.

Microsoft OAuth: **BLOCKED**. Production `.env` has no `MICROSOFT_OAUTH_CLIENT_ID`, `MICROSOFT_OAUTH_CLIENT_SECRET`, or `MICROSOFT_REDIRECT_URI`. `SECRETARY_CREDENTIAL_KEY` is present. Credentials were not invented. Expected redirect after operator Entra setup: `https://web-itx.duckdns.org/secretary/auth/teams/callback`. Required delegated permissions: `openid profile offline_access User.Read Chat.Read ChatMessage.Send`. Work/school only; consumer accounts must not be enabled. Live inbound Teams object: not executed. Recurring Teams sync: not executed. First-turn explicit-send → Pending Action Plan: not executed. No-write-before-approval: not executed. Real Secretary-originated Teams send: **not executed, not claimed as production-tested** (activation blocked; not a user-deferred write after passing OAuth). Docs-only record of this deploy is not deployed. Do not start Graph Refinement or any next phase.
